...
struct quad_tree{
	...
	void set(int x1, int y1, int x2, int y2, int b){
		...
		// 	if ( (x1 == 0) && (y1 == 0) && (x2 == (1<<height)-1) && (y2 == (1<<height)-1)){ 
		// 		//another base case.
		// 		value = b;
		...
		// bool x1r = ( 0<=x1 && (x1<(1<<(height-1))) );
		// bool x2r = ( 0<=x2 && (x2<(1<<(height-1))) );
		// bool y1r = ( 0<=y1 && (y1<(1<<(height-1))) );
		// bool y2r = ( 0<=y2 && (y2<(1<<(height-1))) );

		// int midleft = (1<<(height-1))-1;
		// int midright = (1<<(height-1));

		// if (x1r && x2r && y1r && y2r) {
		// 	topleft->set(x1, y1, x2, y2, b );
		// }
		// if (!x1r && !x2r && y1r && y2r) {
		// 	topright->set(x1-midright, y1, x2-midright, y2, b );
		// }
		// if (x1r && x2r && !y1r && !y2r) {
		// 	bottomleft->set(x1, y1-midright, x2, y2-midright, b );
		// }
		// if (!x1r && !x2r && !y1r && !y2r) {
		// 	bottomright->set(x1-midright, y1-midright, x2-midright, y2-midright, b );
		// }
		// if (x1r && x2r && y1r && !y2r) {
		// 	topleft->set(x1, y1, x2, midleft, b );
		// 	bottomleft->set(x1, 0, x2, y2-midright, b );
		// }
		// if (x1r && !x2r && y1r && y2r) {
		// 	topleft->set(x1, y1, midleft, y2, b );
		// 	topright->set(0, y1, x2-midright, y2, b );
		// }
		// if (!x1r && !x2r && y1r && !y2r) {
		// 	topright->set(x1-midright, y1, x2-midright, midleft, b );
		// 	bottomright->set(x1-midright, 0, x2-midright, y2-midright, b );
		// }
		// if (x1r && !x2r && !y1r && !y2r) {
		// 	bottomleft->set(x1, y1-midright, midleft, y2-midright, b );
		// 	bottomright->set(0, y1-midright, x2-midright, y2-midright, b );
		// }
		// if (x1r && !x2r && y1r && !y2r) {
		// 	topleft->set(x1, y1, midleft, midleft, b );
		// 	topright->set(0, y1, x2-midright, midleft, b );
		// 	bottomleft->set(x1, 0, midleft, y2-midright, b );
		// 	bottomright->set(0, 0, x2-midright, y2-midright, b );
		// }
		// //deleting useless childrens....
		// check();
		// // if(topleft) setnum1node() ;
...
////////
		...
		if(topleft==NULL){
			...
			if(value!=b){
				int k=(1<<(height-1))-1;
				if( (x1==0  &&  x2==2*k+1  &&  y1==0  &&  y2==2*k+1) || height==0 ){
					value=b;
				}			
				else{
					topleft=new quad_tree(value,height-1);
			...
			int k=(1<<(height-1))-1;
			if(x1==0  &&  x2==2*k+1  &&  y1==0  &&  y2==2*k+1){
				value=b;
				delete topleft;
				delete bottomleft;
				delete topright;
